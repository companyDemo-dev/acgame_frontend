        ...
        <div class="scroll-wrapper">
          <div class="scroll">
            ...
            <ul ref="list" class="foods-wrapper" style="min-height: 95vh">
              <li v-for="item in listss" class="food-item border-1px">
                ...
                <div class="dis_flex" @click.stop.prevent="toGoodsDetail(item)" style="width:100%">
                  ...
                  <div v-if="item.state == '1'" class="content">
                  ...
                  <div v-if="item.state == '3'  && item.winnerId != uid" class="content">
                    ...
                    <div style="margin-top: 5px;">
                      <div class="extra">
                        <span class="count" style="margin:0">个人记录：我参与了</span><span style="color: #f01414;">{{item.myPortionNum || 0}}</span>人次
                      </div>
                    </div>
                  </div>
              ...
              <li v-if="listss.length == 0" style="text-align:center;font-size:14px;color: #ccc;">
                <div style="">
...
export default {
  ...
  computed: {
    listss: function() {
      return this.sortKey(this.lists, 'state')
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async loadData() {
      ...
      let {
        products
      } = await getMyProducts_records(obj)
      if (products != null) {
        if (products.length > 0) {
          products.forEach((item) => {
            ...
            item.progress = parseInt(accounting.formatNumber((item.currentPortion / item.totalPortion), 2) * 100)
    ...
    toGoodsDetail(data) {
      ...
      if (data.state == '2') {
        goodsValue = '2'
      } else if (data.state == '3' && data.winnerId == data.uid) {
